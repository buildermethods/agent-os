...
- **File references** — Where this skill emits a reference to a standards file, use your harness's file-reference syntax if it has one (Claude Code uses `@path/to/file.md`). A plain relative path works in every harness.
...
Invoked with no arguments, it analyzes context and suggests relevant standards.

### Explicit Mode (with arguments)

```
api                           # All standards in api/
api/response-format           # Single file
api/response-format api/auth  # Multiple files
root                          # All standards in the root folder
root/naming                   # Single file from root folder
```
...
Before injecting standards, determine which scenario we're in. Read the current conversation and check whether you are in a planning-only mode.

**Three scenarios:**

1. **Conversation** — Regular chat, implementing code, answering questions
2. **Creating a Skill** — Building a `SKILL.md` (in `.claude/skills/`, `.agents/skills/`, `.pi/skills/`, `~/.codex/skills/`, or a plugin's `skills/`)
3. **Shaping/Planning** — Building a spec, e.g. while following `agent-os-shape-spec`

**Detection logic:**

- If you are in a plan / read-only planning mode (Claude Code plan mode) OR the conversation clearly mentions "spec", "plan", "shape" → **Shaping/Planning**
- If the conversation clearly mentions creating a skill, editing a skills directory, or building a reusable procedure → **Creating a Skill**
- Otherwise → **Ask to confirm** (do not assume)

**If neither skill nor plan is clearly detected**, ask:
...
```
No standards index found. Run the agent-os-discover-standards skill first to create
standards, or agent-os-index-standards if you have standards files without an index.
```
...
Match index descriptions against the context. Present suggestions:
...
First, ask how to include the standards:
...
When in conversation scenario, check whether the project has a skills directory that contains related skills. Check whichever of these exist:

- `.claude/skills/` (Claude Code)
- `.agents/skills/` (Agent Skills standard — read by Codex and pi)
- `.pi/skills/` (pi)
...
- **Be specific** — If you know which standards apply, pass them as arguments
...
This skill is used internally by `agent-os-shape-spec` to inject relevant standards during planning. You can also invoke it directly anytime you need standards in context.
