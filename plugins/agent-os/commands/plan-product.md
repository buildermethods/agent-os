...
**If the tech-stack standard exists**, read it and ask:
...
**If no tech-stack standard exists** (or they chose option 2 above), ask:
...
You can edit them directly or run this skill again to update.
```
...
- The `agent-os-shape-spec` skill reads these files when planning features, so having them populated helps with context
